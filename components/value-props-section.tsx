        ...
        >
          {benefits.map((benefit, i) => (
            <motion.div
              key={i}
              variants={item}
              className="group text-left p-5 md:p-6 bg-[#1F2328] rounded-xl md:rounded-[2rem] border border-white/5 hover:border-[#C1A05E]/30 hover:shadow-xl hover:shadow-[#C1A05E]/10 transition-all duration-300"
            >
              <div className="w-10 h-10 md:w-12 md:h-12 rounded-lg md:rounded-xl bg-[#C1A05E]/20 flex items-center justify-center mb-4 group-hover:bg-[#C1A05E] transition-all duration-300">
                <benefit.icon className="h-5 w-5 md:h-6 md:w-6 text-[#C1A05E] group-hover:text-white transition-colors" />
              </div>
              <h3 className="font-semibold md:font-bold text-base md:text-lg text-white mb-2 tracking-tight group-hover:text-[#C1A05E] transition-colors">{benefit.title}</h3>
